      ...
      <div className="container py-1">
        <div className="flex items-baseline justify-between">
          ...
          <div className="flex items-center leading-5 sm:gap-1">
            <div className="hidden items-center text-sm leading-5 sm:flex sm:gap-1">
              {headerConfigs.navLinks.map((link) => (
                <CustomLink
                  href={link.href}
                  key={link.title}
                  className="block p-4 uppercase tracking-widest text-gray-900 transition-colors hover:text-primary"
                >
                  {link.title}
                </CustomLink>
              ))}
